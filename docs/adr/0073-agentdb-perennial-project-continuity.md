...
|---|---|---|
| 2026-09-19 | The read-only SessionStart fast path now accepts the observed `memory_entries` shape with exactly one additional `session_id` column, while continuing to reject every other schema difference. The live global Ruflo 3.42.4 DDL does not define this column; it is recorded as a local compatibility shape, not an upstream Ruflo guarantee. The continuity restore now enforces the same 1,000 ms ceiling declared by the SessionStart stage budget. | The canonical project DB has this column and otherwise matches the pinned 18-column schema (`user_version=0`). The reader selects only `key`, `namespace`, `content`, and `status`, so ignoring this unprojected column preserves the read-only contract; all required columns and the version must still match. Focused unit coverage proves the observed shape reads and an additional unknown `tier` column still falls back. The creator of the local column was not identified. A live read-only SessionStart probe found seven progression rows and completed the production path in 123.6 ms, but it surfaced `UNKNOWN` because the current 8 KiB context bound rejects the 16,347-byte restored checkpoint. A diagnostic-only 65,536-byte bound restored that exact state in 81.9 ms. Payload-bound recovery remains open; the bound was not raised. |
