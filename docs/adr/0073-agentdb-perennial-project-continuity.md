...
| 2026-08-22 | Upstream Ruflo commit `a0262e84` was built and its structural pagination was proven against real CLI pages, but it is not installed in global Ruflo `3.38.19`; its export/import path still returns zero rows on a fresh-DB round trip. | S11 is partially de-risked for structural pagination but remains blocked for cross-host acceptance until the source-bound export backend is fixed or the acceptance contract is revised. Evidence: `docs/reviews/adr-072-s11-upstream-pagination.md`. |
