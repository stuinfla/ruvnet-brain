...
|---|---|---|
| 2026-09-19 | The read-only SessionStart fast path accepts the observed `memory_entries` shape with exactly one additional `session_id` column, while rejecting every other schema difference. The live global Ruflo 3.42.4 DDL does not define this column; it is a local compatibility shape, not an upstream Ruflo guarantee. The continuity restore enforces the same 1,000 ms ceiling declared by the SessionStart stage budget. Oversized full checkpoints now receive a deterministic bounded-summary attempt before the existing UNKNOWN result. | The canonical project DB has the extra column and otherwise matches the pinned 18-column schema (`user_version=0`). The reader selects only `key`, `namespace`, `content`, and `status`; all required columns and the version must still match, and an unknown `tier` column still falls back. The creator of `session_id` was not identified. A read-only production-path probe found seven progression heads and returned `restored-summary` with warning severity in 278.6 ms at 6,793 bytes under the unchanged 8 KiB bound. It preserved the exact current goal and next action and marked 21 omitted conflict values with SHA-256 `b5da2ccb7ddbd447b353970f5e0dd924e16f4f475f241642e25341d98613c706`; the exact head keys still address canonical records. A test with oversized mandatory goal/action remains UNKNOWN and injects no partial state. |
