...
| 2026-08-27 | Dream Cycle 2026-08-27, DEEP=enforcement-integrity. `plugin/scripts/lesson-hooks.sh`'s `ship`-trigger dispatch (line ~209) required a literal single space between `npm`/`yarn`/`pnpm`/`gh` and their verb, so a tab, doubled space, or wrapped newline in the real Bash command text (plausible from templated/agent-generated commands) was silently NOT recognized as shipping — while `plugin/scripts/degradation-watch.mjs`'s independent `DEPENDENT_COMMANDS` definition of the same event (already `\s+`-tolerant) correctly caught it, so the memory-durability safety refusal fired but the ship lesson dispatch did not, for the exact same command. Widened the bash-side pattern to `[[:space:]]+` and normalized embedded newlines/tabs to spaces via `tr` before matching (grep matches one line at a time, so a real newline would otherwise split the command across two internal lines and never match regardless of the pattern). | `tests/unit/lesson-gate.test.mjs`'s existing `describe('the two ship definitions agree', ...)` — the parity test this repo's own comments say exists specifically to prevent this drift — passed on unmodified `main` with 7 cases, none carrying irregular whitespace. Extended with 4 new cases; reverting only the `lesson-hooks.sh` pattern reproduces `AssertionError: lesson-hooks.sh: npm  publish: expected false to be true`; all 61/61 tests in the file pass restored. PR: dream/2026-08-27-enforcement-integrity. |
