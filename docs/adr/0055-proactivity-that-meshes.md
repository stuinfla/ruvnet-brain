...
## Currency log
| 2026-08-30 | SessionStart now emits factual health/action state only and suppresses the active footer when the brain is off. | `plugin/scripts/session-start-core.mjs` and `tests/unit/session-start-core-parity.test.mjs` close the noisy-hook path reported by the user. |
