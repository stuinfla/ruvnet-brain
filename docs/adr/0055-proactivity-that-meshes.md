...
- **Interface enforcement moved to structure, not reconstructed Bash.** Commit `e089074` adds
  ...
  prerequisite. Commit `4ad464e` made `plugin/scripts/verify-interface.sh` advisory-only; the shell
  ID is now retired as a silent compatibility no-op; raw Bash cannot produce an interface refusal.
...
| **Execution** (Bash / Task / Agent) | structured MCP boundary for managed CLI calls; existing registered execution guards for the remaining walls | route-dispatch, design-wall, version-bump, package/network checks; legacy verify-interface ID is silent compatibility only |
...
2. **Hook battery v2** before changing any registration. Red first: held-open stdin exceeds
   ...
   **SHIPPED 2026-07-27 (`scripts/selfcheck.mjs`, `tests/unit/selfcheck-battery.test.mjs`, 32
   ...
   `learn-flush`, `route-dispatch`, `design-wall`, historical `verify-interface`, `protect-brain-state`,
...
| 2026-08-12 | **The flywheel advisory now distinguishes CONFIGURED from OPERATIONAL (issue #138). No hook, threshold, silence rule or dial changes — one advisory's PREDICATE and copy do.** | `ground-ruvnet.sh` treated any occurrence of `RUFLO_HARNESS_LOOP` in `.claude/settings.json` as proof the flywheel was on, then went silent. Verified against rUv's source at `f35c545`, not recalled: `harness-worker.ts:41` tests `process.env.RUFLO_HARNESS_LOOP` inside the DAEMON process, and `:54` returns `reason: 'opt-in required'` when it is absent — so a settings file proved only that a file mentions a name. The result was a false ENABLED, which is worse than a false disabled because nobody goes looking. The detector now has three states (off / declared / on) and only the process environment proves the third; the advisory fires on `declared` and says "Configured is not operational". The guidance also gained the second, non-optional gate — the hash-pinned project anchor `.claude/eval/flywheel-anchor.manifest.json` (`harness-project-anchor.ts:186`, `:32`; ruflo #2840 / PR #2848) — without which a downstream repo fails closed, so the previous "TURN IT ON" could not produce a working setup. Both data gates are now stated (12 patterns to harvest, 20 held-out tasks to generate) and the user is told how to VERIFY (`ruflo hooks intelligence --status`) rather than trust a proxy. Same shape as #136: a surface reporting a proxy as if it were the answer. Guarded by `tests/unit/flywheel-configured-vs-operational.test.mjs`, mutation-proved — restoring `FLYWHEEL=on` for the declared case fails the TEETH case. |
| 2026-08-10 | **PreToolUse speech now routes through the decision gate; no hook, threshold, silence rule or dial changes.** | ADR-067 made one process the sole author of a refusal AND the forwarder of the advisory envelope on the write path, so there is now one place that can speak or refuse instead of two racing. The mesh's own producers are untouched; `tests/integration/unprompted-speech-registry.test.mjs` still proves no bare producer can be wired. Side effect worth recording: the gate's anchored matchers retired two known F4 defects (`Write|Edit|Bash` and bare `Bash` were unanchored and selected tools nobody chose). |
| 2026-08-10 | `plugin/scripts/lesson-bridge.mjs` gained a Windows-safe entrypoint guard. **No hook, threshold, silence rule or dial in the mesh changes.** | The file is a CLI run by the nightly, not a hook, and the change is confined to how it answers "am I the entrypoint" — `fileURLToPath` instead of `URL().pathname`, and a `try` around `realpathSync`, matching `hook-input.mjs:538`. It fired as `ENOENT: lstat 'D:\D:'` in windows-unit, failing the importing SUITE rather than any assertion. Swept across every shipped `.mjs` and pinned by `tests/unit/entrypoint-guard-safety.test.mjs`; nothing the mesh emits, suppresses, or times was touched. |
| 2026-08-06 | The continuation gate gained two SOURCES — open PRs and GitHub security alerts. No hook, threshold, silence rule or dial in the mesh changes; the gate simply stops being blind to two kinds of outstanding work. | The gate had three inputs: ledger items registered via `--commit-to`, issue-SLA breaches, and red CI. PRs #117, #118 and #119 were opened, every required check passed, and at the Stop boundary it said nothing — correctly by its own logic, since `open` was genuinely empty. The `|| true` on its registration is NOT the cause and is harmless: it delivers via `additionalContext` and exits 0 (`continuation-gate.mjs:336-344`), never via its exit code. `openPrWork()` treats green as merge-it and red as fix-it, and keeps `pending` SILENT because a run in flight is not yet evidence — the same discipline `redCiOpenWork()` already applies. `securityAlertWork()` surfaces Dependabot/secret-scanning/code-scanning at HIGH+CRITICAL only, with secret-scanning always, because the owner had been relaying those from GitHub emails by hand. Producer is `issue-watch.mjs`, which already writes the artifact the gate reads — no second writer, no network call at the Stop boundary. Proven live: it wrote `prs:[#119:failing]` and that is how the release-qe orphan on #119 was found. 25/25 across all three gate suites. |
| 2026-08-06 | `anticipate.sh`'s hard watchdog now forks its `sleep` as its OWN child and reaps it. No behaviour in the mesh changes; what changes is that the hook leaves no process behind. | Waking L4 made the watchdog reachable for the first time on a real install, and `release-qe` immediately failed the orphan contract in BOTH stdin regimes: `left descendants alive after SIGTERM to its process group`. `kill` signalled the subshell while the `sleep` it forked was orphaned; `selfcheck.mjs` §5 uses `WATCHDOG_GRACE_MS = 2000`, the SAME 2000ms as the sleep, so it always outlived the probe by the spawn delta — deterministic on ubuntu, green on macOS, and both CI runs failed identically. Measured with unique markers on a fresh baseline: old form leaves 1 orphaned sleep, new form leaves 0. A `trap`-based variant was tried FIRST and measured WORSE (it defeats the shell's exec optimisation and leaked where the original did not); it was reverted rather than shipped. |
| 2026-08-06 | The proactive mesh's own modules moved INTO the payload (`plugin/scripts/`), and `scripts/` now holds `export *` shims. The mesh contract is unchanged — every hook, threshold, silence rule and dial behaves identically; what changed is that on a real install they now EXIST. | ADR-065. Only `plugin/` reaches a user (`marketplace.json` `"source": "./plugin"`; `stagePayload()` copies it verbatim) and every shipped layout flattens it, so `goal-match.mjs`, `capability-registry.mjs` and `advocacy-outcomes.mjs` — which lived only in repo-root `scripts/` — resolved to nothing. Measured on an install-shaped flattened fixture with no module env vars: `main` → `anticipate.sh --status` prints `advocacy-outcomes module not found at <fixture>/../../scripts/advocacy-outcomes.mjs` (the `$SELF_DIR/../..` overshoot); after the move the same fixture renders the real dismissal ledger. `unprompted-runtime.mjs:85` carried the identical `CODE_ROOT` overshoot for `user-settings.mjs` (the 1–5 dial) and `advocacy-outcomes.mjs` (the ledger), both of which `catch` to defaults — so the dial was silently unread on every real install; it now resolves from `SCRIPTS_DIR`. Guarded by `tests/unit/payload-self-contained.test.mjs`, proven to fail by removing a module from the payload: `plugin/scripts/anticipate.sh → goal-match.mjs … degrades to SILENCE`. |
| 2026-08-06 | **REVERTED, not fixed.** The `readable`-drain approach to the Windows held-open pipe is withdrawn from `hook-input.mjs`; `readStdinBounded` stays exactly as it is on `main`. Two independently-measured failures, in opposite directions, from the same root cause. | **Attempt 1 (`onData` on the read chunk) FAILED OPEN.** `read()` itself emits `data`, so every byte was counted twice: a 79-byte envelope returned as **158 bytes** of the payload concatenated with itself, `JSON.parse` threw, every gate field read `''`, and the PreToolUse wall waved commands through on every Windows invocation. CI surfaced it only as 30 downstream assertions in `codex-lifecycle-hooks` and `continuation-gate` (`expected '' to contain 'ruvnet_cli_help'`), run `30858227845`. **Attempt 2 (drain only) HUNG.** Measured against a held-open pipe with `platform` forced: `main` exits in 118ms, the drain-only version **never exits (>2000ms)** — reproducing `routing-outcome-capture.test.mjs > the registered observer exits when the host leaves stdin open`, `expected true to be false` (i.e. `timedOut`), windows-unit job `92610172373`. **Common root cause:** attaching a `readable` listener pins stdin in paused mode permanently — `resume()` computes `flowing = !readableListening` and cannot undo it — so the drain becomes the only byte path AND the stream's exit lifecycle changes. That is a lifecycle change, not a parsing tweak, and it cannot be corrected by a one-line edit. The original intermittent `SessionStart: waited for stdin EOF` symptom remains OPEN and needs a design that does not attach `readable` to `process.stdin`. Recording the two measurements so the next attempt does not rediscover them. |
| 2026-08-03 | Bounded the shared hook-input parser on Windows inherited pipes by draining readable bytes and polling until the existing idle/empty deadlines. | Post-merge main CI run `30847567626` failed only `swarm-slot-recycler.test.mjs` on Windows because the held-open envelope was not surfaced through `data`. **Superseded by the 2026-08-06 row: this approach was measured to fail in both directions and was reverted; it never reached `main`.** |
...
|---|---|---|
| 2026-08-19 | **Two governed hooks changed later the same day, and BOTH moved TOWARD this document.** | (1) The timeout rule in §Gate requires `timeout <= 5s`; the two decision-gate PreToolUse entries had been at 10s since 2026-08-14 and are now back to 5s, so `hooks.json` is compliant with this ADR again — re-measured ~415ms through the full registered path `hook-shim.mjs decision-gate write`, twelve times the headroom (ADR-067, 2026-08-19). (2) `ground-ruvnet.sh` was emitting literal backslashes into the injected prompt: its MEMORY DIAGNOSIS guidance escaped its backticks inside a QUOTED heredoc (`cat <<'EOF'`), where nothing is substituted, so every memory-diagnosis turn shipped `\\`ruflo memory retrieve ...\\`` verbatim to the model. Advisory stdout content only — exit codes, matchers and failure modes unchanged (`tests/unit/issue-66-memory-detection.test.mjs`). |
| 2026-08-19 | **Two of my own regressions caught by the integration suite, both from conflating states.** | (1) `ingest-repo.test.mjs` required `--out \\.` — the tool's own cwd — which was the READER/WRITER SPLIT: ingest wrote to the checkout while forge-mcp-all reads `storeRoot()`. That split is why three repos ingested cleanly on 2026-08-12, each printed "searchable now", and none could be found. The assertion now checks the RELATIONSHIP (`--out` equals the root retrieval reads) rather than a literal, because a test that pins a hardcode defends the defect — the same correction #139 forced on the learner-scope test. (2) `learn-capture.sh` was made to require an existing `.swarm` to stop it planting one in every repo the user opened; that ALSO broke a legitimate first capture in a project that had explicitly opted in, and `learning-scope-policy` went red. Presence of `.swarm` answers "has Ruflo run here", not "did this project ask for learning". The discriminator is now whether the scope was CONFIGURED rather than defaulted — and it had to move BEFORE the preferences fallback, because `runtime-preferences --learning-scope` RESOLVES the default and returns "project" against an empty config root (measured), so reading it afterwards reported "configured" always and let the mutation straight back in. Both invariants now hold together: 37/38 integration files, 260 passing. |
...
| 2026-08-13 | **Cross-project misbehaviour in two hooks on this surface, fixed; and the coarse `governs:` list predicted in the previous row has now blocked twice in one evening.** | `degradation-watch` was probing `cwd/.swarm/memory.db` machine-wide, creating a `.swarm` directory and writing a probe row into ANY repo the user was standing in (ADR-058 D5: never touch what we do not own) — now a scratch db in tmpdir, which also closes the absent-store hole. `identifier-preflight` treated one configured Codex model as a complete allowlist and would have falsely refused `o3`; it now refuses only a near-miss typo. Both found by an adversarial hook audit rather than by a test, which is the real finding: NOTHING IN CI RUNS THESE HOOKS IN A PROJECT THAT IS NOT THIS ONE, on either host. That gap is being closed with a conformance gate rather than another currency row. |
| 2026-08-13 | **Two more refusal policies now sit on the hook surface this ADR governs: `identifier-preflight` (bash) and `adr-currency` (write).** | Both close SILENT failures rather than adding friction. `identifier-preflight` refuses a command naming a model this machine's CLI does not accept — `codex exec --model gpt-5.6` (correct: `gpt-5.6-sol`) printed a 400 and EXITED 0 into a redirected file, so a 50-minute audit produced nothing and there was no exit code to catch. `adr-currency` moves the ADR check from push-time to edit-time: it refuses writing MORE code governed by a document already stale from the last round, because a gate at the end cannot shape work, only penalise it, and it trains running at the wall. Both fail OPEN on anything they cannot positively determine, which is the constraint an adversarial review imposed after finding a sibling hook turning a missing `sqlite3` into a confident claim that the memory store was corrupt. NOTE FOR A FUTURE PASS, surfaced by this very check: this ADR governs ~70 paths, so essentially any hook change makes it presumed-stale and every such change lands here. That is a signal the `governs:` list is too coarse to be informative, not that the rule is wrong. |
| 2026-08-13 | **The `ship` trigger reached a real push for the first time, and two policies joined the refusal gate.** | `lesson-hooks.sh` carried `PreToolUse-push) TRIGGERS="ship"` and nothing has ever sent that token: hooks.json emits only `unprompted-speech UserPromptSubmit`, and `decision-gate.mjs` maps every PreToolUse to exactly `PreToolUse-bash` or `PreToolUse-write`. So `git push` always arrived as `mutate-machine`, and FIVE ratified ship lessons had never fired once — including `G-remote-ci-gates-shipping` and `G-test-the-artifact-not-the-checkout`, the two that would have spoken before a test shipped that broke windows-unit and red-lit an unrelated dependabot PR. `ship` now rides the bash payload, whose command text this dispatcher already extracts. Proven at the process boundary: `git push` and `npm publish` deliver, `ls`/`git status`/`grep` do not. **OPEN, from an adversarial review:** the match is a glob over raw command text — a proxy, and the exact mistake this session is about. `git -C <abs> push` matches nothing while `grep -n "npm publish" docs/` matches; the repo owns the correct instrument (`hook-input.mjs findInvocations()`, executable position, "never a grep"). Also unfixed: `wired-check`'s Check C still greps this file's case labels, so deleting the dead `PreToolUse-push` branch would make the audit report ship inert while it actually fires. |
| 2026-08-08 | **Re-read after the #125/#126/#127 Console-honesty fixes; the proactive mesh is unchanged.** | Three governed surfaces moved, all for the same reason and none of them a mesh change. `plugin/scripts/capability-registry.mjs` stops reporting quarantined imported-owner lessons as "awaiting your ratification" (they can never be ratified; it now delegates to the store's own `pending()`). `plugin/scripts/update-apply.mjs` treats an ACTIVE spine already at or above the requested version as converged instead of a failed sync — selection stays EXACT so issue #64 does not regress, and a test now asserts that in source. `scripts/onboarding-console.mjs` resolves the project memory store by asking the filesystem rather than hard-coding one filename. Each removes a false statement from a surface; none alters a hook, a threshold, a channel, or what the mesh may say. |
...
| 2026-08-27 | Dream Cycle 2026-08-27, DEEP=enforcement-integrity. `plugin/scripts/lesson-hooks.sh`'s `ship`-trigger dispatch (line ~209) required a literal single space between `npm`/`yarn`/`pnpm`/`gh` and their verb, so a tab, doubled space, or wrapped newline in the real Bash command text (plausible from templated/agent-generated commands) was silently NOT recognized as shipping — while `plugin/scripts/degradation-watch.mjs`'s independent `DEPENDENT_COMMANDS` definition of the same event (already `\s+`-tolerant) correctly caught it, so the memory-durability safety refusal fired but the ship lesson dispatch did not, for the exact same command. Widened the bash-side pattern to `[[:space:]]+` and, in the first cut, normalized embedded newlines/tabs to spaces via `tr` **after** `sed`'s quote-stripping. **An independent adversarial critic pass caught that first cut reopening the same defect class in a new shape**, live-reproduced through the real dispatcher: `sed` (no `-z`) is line-oriented, so a double-quoted string that legitimately SPANS a real newline (a multi-line commit message) was never stripped — its open/close quotes landed on different sed-internal "lines" — and running `tr` afterward then collapsed that leftover embedded newline into a space, joining words like "npm"/"publish" that were meant to stay inside the quotes into an unquoted, matching phrase. Fixed by reordering: `tr` now runs BEFORE `sed`, so the whole command becomes one logical line first and the existing quote-stripping regexes span it correctly, same as they already did for single-line quotes. | `tests/unit/lesson-gate.test.mjs`'s existing `describe('the two ship definitions agree', ...)` — the parity test this repo's own comments say exists specifically to prevent this drift — passed on unmodified `main` with 7 cases, none carrying irregular whitespace. Extended with 4 whitespace-variant cases (reverting only the `lesson-hooks.sh` pattern reproduces `AssertionError: lesson-hooks.sh: npm  publish: expected false to be true`) plus 1 TEETH case for the critic-caught regression (`git commit -m "release notes\nnpm\npublish is unaffected by this change"` must stay `false`); all 61/61 tests in the file pass on the corrected candidate. Regression suite (`test:unit` 3403/3592 pass, `test:integration` 240/313 pass) byte-identical to the pre-existing baseline both before and after the reorder. PR: dream/2026-08-27-enforcement-integrity. |
