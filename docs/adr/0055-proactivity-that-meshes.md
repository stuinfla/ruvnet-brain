...
| date | why |
|---|---|
| 2026-08-30 | Rechecked plugin/scripts/session-start-core.mjs and plugin/scripts/ground-ruvnet.sh in 05cabf0: build/playbook and response-footer injection are disabled while factual grounding/drift gates remain. |
| 2026-08-22 | **RuvNet capability-claim truth is now a source-bound Stop-gate input; the existing mesh thresholds, consent boundary, and unrelated open-work sources are unchanged.** | ADR-074 adds `capabilityClaimWork()` to `plugin/scripts/continuation-gate.mjs`: a claim contradicted by fresh typed evidence, or an absence/behavior/version/health claim whose required receipt is incomplete, is surfaced as `capability-claim-integrity`; a claim matching the exact receipt stays silent. `kb/forge-evidence.mjs` now binds source observations to the full commit SHA rather than a short display prefix. `tests/unit/continuation-gate-capability-truth.test.mjs` pins the critical distinction: unsupported assertions become `UNKNOWN`, not false facts, while exact installation, source-behavior, current-version, and health evidence can clear their respective claims. Latest-version assertions remain `UNKNOWN` until a registry receipt exists. |
...
|---|---|---|
| 2026-08-22 | Re-read the governed proactive hook mesh after the eight-process convergence changes; the decision law, host registries, bounded dispatch, and fail-open boundaries remain aligned. | Exact candidate `5ecb15f8c6c3cabce07b54e4f773a31a7385d6fe` changes continuity, capability evidence, MetaHarness gating, and their receipts; no governed hook semantics changed. This currency row satisfies the pre-push drift gate without weakening its scope. |
| 2026-08-22 | **Exact file and path questions can no longer terminate in the generic capability-card answer.** | A live query naming `ruvnet-brain/scripts/nightly-wrapper.sh` and `scripts/self-update.mjs` returned only `kb/capability-cards.md#ruvnet-brain`: the router correctly named the repository, but the answer fast lane treated that broad card as sufficient and never searched the requested artifacts. `answerFromCards()` now falls through on path-shaped or source-file-shaped queries, preserving the card as a routing hint while forcing full source retrieval for the answer. `card-lane.test.mjs` pins the failure shape. This is a routing correction, not a claim of 100% semantic recall. |
