...
## Currency log
| 2026-09-12 | Currency review at commit 5f919c52: decision unchanged — the executable contract is intact, and one of its runtime checkers is now actually installed. The installed-update proof boundary (row 2026-09-07) runs through `kb/forge-update.mjs` `loadTrustedCoverageValidator()`, which demands `KB_DIR/coverage-integrity.mjs`; measured 2026-09-12, no production path placed it (`scripts/build-bundle.mjs` walks static imports and cannot see the dynamic load; `bin/install.mjs` imported the module for its own checks only; the sole copier was `tests/unit/forge-update-apply-rollback.test.mjs:95`, into a fixture AND every fixture bundle), so that boundary had never executed on a customer brain — every 4.3.21 `--update` died and fell back to a fresh install. `bin/install.mjs` (`5f919c52`) now places the package validator in `unzipInto()` and before the `--update` child; `kb/forge-update.mjs` (`5f919c52`) `prepareCandidate` carries the live copy forward. The fixture now obtains the validator the way production does and its bundles no longer ship it; 9 + 16 tests RED→GREEN, 208 green across the install/forge-update suites. Also in the window: `kb/forge-update.mjs` `9fda9e1b` (overlay capture `{ strict: false }` root walk). | Reviewed both `kb/forge-update.mjs` diffs and the `bin/install.mjs` diff in full; `kb/update-storage-transaction.mjs`, `kb/refresh-run.mjs`, `scripts/release-transaction*.mjs`, `scripts/release.mjs`, `scripts/build-bundle.mjs` and the three `docs/ddd/00{18,19,20}-*` contexts did not move. reviewed_digest cc62a96b4a3d. |
| 2026-09-12 | Currency review at commit 7b472440: decision unchanged — the lifecycle context still proves itself through the refresh-run receipt, and the receipt WRITER (`kb/refresh-run.mjs`, untouched) still appends phases as they complete and stops at the first required failure. The READER learned to describe that designed shape: `plugin/scripts/nightly-scheduler.mjs` gained `describeFailedRefreshRun()` and `refreshRunHealth` now reports a FAILED prefix-shaped receipt as `failed at <phase>: <first line of updateResult.reason, ≤200>` instead of "invalid: phase ledger differs from its declared contract" (measured 2026-09-12 on receipt `1789208177087-d51ad2c7e6b004b2`: cause "unresolved rollback state exists…" was carried in the receipt and surfaced nowhere); `schedulerStatus` (darwin) now exposes `lastExitCode` so `scripts/nightly-watchdog.mjs productSchedulerVerdict` projects a loaded, verified, fired-and-failed job as FAILING/STALE from its receipt instead of MISSING. `validateRefreshReceiptEnvelope` — the two-run PROOF's contract — is byte-identical; the immutable runner (`nightly-refresh-<sha>.mjs`) contains none of this code, so registered digests are unaffected. | Reviewed `plugin/scripts/nightly-scheduler.mjs` (`describeFailedRefreshRun`, `refreshRunHealth`, `schedulerStatus`), `kb/refresh-run.mjs` (writer contract, unchanged), `scripts/nightly-watchdog.mjs`; tests `tests/unit/nightly-refresh-run-health.test.mjs` (7 RED → 9 GREEN) and `tests/unit/nightly-watchdog.test.mjs` (4 RED → 34 GREEN). reviewed_digest f6d5e94f1794. |
| 2026-09-11 | Currency review at commit 7296c984: decision unchanged — the seven delivery contexts and their one-way dependencies are intact. `scripts/release.mjs` rewritten and restored (ADR-0070's row; net 0 vs 85f584b2). `kb/update-storage-transaction.mjs` `91b6a3d6`: a tooling symlink inside the managed tree (`kb/node_modules/.bin/semver`) is now reported in the storage inventory as `symlinkCount` instead of throwing at `forge-update.mjs:1141`; a symlinked `.rvf` or an escaping link still throws. `kb/forge-update.mjs` and `bin/install.mjs` `dc18fadc` (retention rule). `scripts/build-bundle.mjs`, `scripts/corpus-candidate.mjs`, `scripts/release-transaction.mjs`, `scripts/release-transaction-provider.mjs`, `kb/refresh-run.mjs`, `kb/brain-profile.mjs`, `bin/nightly-refresh.mjs`, `plugin/scripts/nightly-scheduler.mjs`, the host adapters and DDD-0018 / 0019 / 0020 did not move. | Reviewed `scripts/release.mjs`, `kb/update-storage-transaction.mjs`, `kb/forge-update.mjs`, `bin/install.mjs`. reviewed_digest 6f994956aa56. |
