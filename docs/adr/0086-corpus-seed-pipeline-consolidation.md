...
governs:
  ...
  - scripts/oracle/source-language.mjs
  - scripts/oracle/source-adapters.mjs
  - scripts/oracle/source-units.mjs
  - scripts/oracle/unit-inventory.mjs
  - scripts/oracle/produce-questions.mjs
  - scripts/oracle/validate-labels.mjs
  - scripts/oracle/spike-run.mjs
...
ADR-087 now owns the comprehensive consistency cleanup. The schema-1 oracle and frozen recall
fixture remain diagnostic or explicitly reduced evidence as stated in the September 15 amendment.
They do not establish the complete C3 source-unit contract or current full-corpus coverage.

As of the September 16 implementation review, the pinned-tree source inventory is wired into
`source-units.mjs` and `spike-run.mjs`. Syntax candidates require exact-blob semantic dispositions;
unsupported syntax and missing review keep U unknown and prohibit sampling. This is implementation
under qualification, not a completed semantic review of the corpus. The question producer retains
oversized or failed units as unproduced, and deterministic validation binds labels to the exact
selected inventory. Acceptance requires the resulting receipts, not the presence of these modules.
...
### Amendment 2026-09-15 — a sealed acquisition manifest replaces round-stability (NORMATIVE)

The decision quoted above required reconciliation to converge on a *stable live observation*: the
loop re-observed the entire source universe after each build round and only accepted when the new
observation hashed identically to the one it started with, failing the generation after three rounds.

**That requirement is retired.** It made progress unreliable under sustained churn, which is the
steady state for this organisation rather than a rare race. Measured 2026-09-14/15: a round takes
about an hour; the observation hash covers each repository's `updatedAt`, `pushedAt`, `diskUsage` and
default-branch `oid`; eight repositories were pushed in 24 hours and thirteen of 185 had moved since
the committed coverage generation. A local run refreshed 90 distinct stores, rebuilt both aggregates,
and then exited 1 with `source observation did not stabilize within 3 reconciliation rounds`. All four
corpus-seed CI runs in history failed, none having reached even that far. A quiet interval could still
succeed — the defect is unreliable progress, not impossibility.

**The replacement, per the Dual deliberation of 2026-09-15 (decision "choose A", verifier
ACCEPT_WITH_CORRECTIONS).** One bounded discovery pass freezes the identity set. Every required source
resolves independently to immutable pinned inputs, and a resolved entry is frozen — movement anywhere
else can never invalidate it or restart the generation. The manifest seals once required acquisition
succeeds, and every later step consumes it exclusively. Acceptance is **completeness against the
sealed manifest**: every required source validated against the inputs it was pinned to, with zero
missing, failed or mismatched required sources. A source that moves mid-run finishes at its pinned
revision and is collected by the *next* generation; `latest` is never substituted and an exhausted
partial generation is never accepted. The closing observation survives only as bounded freshness
telemetry — `NO_CHANGE_OBSERVED`, `NEWER_REVISION_OBSERVED` or `UNKNOWN` — which cannot modify the
manifest or veto acceptance.

**What this changes in the claim.** "Collected every source requiring ingestion" now means *in this
recorded discovery manifest, at its pinned revision, under the recorded policy*, with every other
discovered identity carrying an evidenced disposition. It is interval-based discovery and acquisition,
not an atomic snapshot of GitHub at an instant, and not a claim of currency at publication. This is a
deliberate retirement of the old freshness condition, not a restatement of it.

**Preserved unchanged:** pruning against sealed membership, exact source bindings and validated reuse
contracts, aggregate provenance and required vector bindings, isolation, cancellation and bounded
...
| 4 | Implement sealed-manifest acquisition and StoreResult integration. Preserve explicitly validated legacy reuse. Pass the sealed coverage into preparation. | **Acceptance AMENDED 2026-09-15** (see the amendment under Decision): no-op stability, source movement and round exhaustion are RETIRED as acceptance tests. Exercised instead: continuous churn cannot restart or invalidate a sealed generation; a gist revision moving mid-fetch retries the same pinned inputs without re-observing; an exhausted partial generation fails explicitly rather than being accepted; freshness telemetry never vetoes. Worker cancellation, removed repositories and legacy provenance limitations remain exercised. | **Merged to main** as `72e7718e` (19:15, "step 4 — real prune, no-live-observe candidate prep, worker cancellation, path isolation"). Worktree original `9a608bf3`. Acceptance crosswalk: F7 and F8 CLOSED in source here (`scripts/corpus-reconcile.mjs:298, :541`). |
...
|---|---|---|
| 2026-09-17 | Reviewed the consolidation plan and governed source paths at integration HEAD; the implementation remains proposed and no full corpus, C3, or public delivery claim is made. | Digest intentionally not stamped: `scripts/oracle/source-language.mjs` and `scripts/oracle/source-adapters.mjs` are untracked in the current checkout, so the governed digest is unresolvable. |
...
| 2026-09-16 | Reconciled active oracle inventory ownership and historical status | `scripts/oracle/source-units.mjs` now consumes the pinned-tree inventory; ADR-087 owns remaining semantic coverage, full corpus census, acceptance, and publication proof. No C3 pass is asserted. |
