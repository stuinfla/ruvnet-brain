...
governs:
  ...
  - tests/unit/gist-git-transport.test.mjs
  - tests/integration/public-gist-git-capture.test.mjs
...
|---|---|---|
| 2026-09-19 | **No-token gist capture now binds the public Git snapshot instead of issuing one REST detail request per gist.** The production path clones each public gist as a bare repository with no checkout, pins the exact Git HEAD commit and tree, inventories every tree blob, and requires every untruncated API-listed filename/size/raw blob ID to match that HEAD. A raw URL revision is treated as the actual file blob identity; it is never called or copied into a fabricated API history version. Gist content is sealed with the verified Git head/tree and per-file blob proof. The current live public inventory was independently observed as 495 gists over 5 pages; all 495 carried an explicit `truncated:false`. True or missing top-level completeness is preserved into the observation and coverage becomes UNVERIFIED; a truncated listing fails capture closed until a future change enriches and re-seals the source observation from the complete Git tree. This explicitly does NOT claim support for >300-file gists. A real public multi-file gist with two distinct observed per-file blob revisions was captured through the no-detail route and passed. | `scripts/gist-git-transport.mjs`; `scripts/gist-receipts.mjs`; `scripts/source-coverage.mjs`; `tests/unit/gist-git-transport.test.mjs`; `tests/integration/public-gist-git-capture.test.mjs`; live inventory receipt `/Users/stuartkerr/Code/.codex-audits/ruos-access-20260919/public-gist-inventory-live.json`; public gist `1afab76a2b67161b7bd1fbdd8930b408` (detail observation + anonymous `git ls-remote`/bare clone). |
