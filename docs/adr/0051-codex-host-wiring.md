...
| 2026-09-05 | **`codex-hook-wrapper.mjs`'s `blockingHooks` had drifted from `hook-shim.mjs`'s own TABLE, undetected.** Independently found and fixed the same night by a concurrent Dream Cycle firing that landed first (this row is retained for the ledger trail; the code fix below is superseded by that landing, not stacked on top of it). | Dream Cycle 2026-09-05 (cross-host-conformance / codex-parity, DEEP+SCAN): `blockingHooks` — the Set that decides which hookId's real exit code the wrapper forwards to Codex, everything else coerced to exit 0 — still listed `'route-dispatch'` as if an exit-2 from it meant something, but `hook-shim.mjs`'s own TABLE has declared `route-dispatch` `mode: 'advisory'` since issue #84 (a refusal there arrives ~140ms too late to be enforcement; hook-shim.mjs's own comment already says the opposite claim "was FALSE and had to go"). Harmless in practice — hook-shim.mjs's own dispatch already coerces route-dispatch's exit code to 0 regardless of what the wrapper believes — but exactly the false-confidence, "wired and toothless" shape of two prior ACCEPTED nightly findings on this file family (#149/#150's mesh census; #197's CONTEXT_EVENTS hand-copy), and nothing tested that these two lists agree. Removed the stale entry; added `tests/unit/codex-blocking-hooks-parity.test.mjs`, which proves every Codex-registered, TABLE-`mode:'blocking'` hookId is in `blockingHooks` (the dangerous direction: a missing one silently turns a real refusal into an allow) and that `blockingHooks` names nothing TABLE does not also call blocking (the misleading direction, this drift's shape). Registration, wrapper timeout/detach behavior, and the adapter are otherwise unchanged. |
...
|---|---|---|
| 2026-08-20 | **Codex hooks were silently no-opping in the one gate that exists to catch that.** | Night 2 of the Dream Machine measured it and I verified it by hand: `codex-hooks.json`'s trampoline is `if (!s?.isFile()) process.exit(0)` against `$RUVNET_BRAIN_HOME/codex-hook.mjs`, and the both-hosts conformance gate deliberately points RUVNET_BRAIN_HOME at an EMPTY dir (to keep real ledgers untouched). Fired by hand with an empty spine: exit 0, **0 bytes stdout, 0 bytes stderr** — the hook never ran, so every Codex 'no stderr / no artifacts / within timeout' assertion passed BY CONSTRUCTION, never by measurement. Claude Code survives the same gap via `hook-shim.mjs`'s LOUD frozen fallback straight to `${CLAUDE_PLUGIN_ROOT}`; Codex has no fallback and depends on an installed Stable Spine, full stop. PR #148 adds a spine fixture so the Codex half of the gate measures something. Separately (#150) the drift mesh in `hook-registry.mjs` never enumerated `codex-hooks.json` at all, so M1/M3/M5/M6 had ZERO automated coverage of this host — both halves of Codex parity were unwatched at once. |
| 2026-08-19 | Model-catalog snapshot refreshed and three drifted OpenAI prices corrected from live. No Codex wiring change; recorded here because the shared `check` job gates both hosts and was red on every open PR until the snapshot was refreshed. |
