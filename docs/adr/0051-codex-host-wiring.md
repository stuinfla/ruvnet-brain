...
|---|---|---|
| 2026-08-03 | Gave the held-open registered-command probe a Windows-only 2s process-start budget while retaining the 250ms wrapper input deadline. | The hosted Windows failure was the first `SessionStart` shell process being killed at the Linux-oriented 750ms outer budget; the assertion itself remains unchanged and still proves completion without stdin EOF. `tests/unit/codex-lifecycle-hooks.test.mjs`, commit `1a1136e`. |
