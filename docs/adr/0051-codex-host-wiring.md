...
|---|---|---|
| 2026-08-22 | Re-read the combined installer and Codex manifest changes; the stable wrapper/adapter registration remains intact and the decision is strengthened. | `4e68453` changes `bin/install.mjs` from immediate stale-generation deletion to PID-incarnation lease proof, so a still-running Codex or Claude session keeps the generation its wrapper resolves. `6336c52` adds the user-level Codex/Claude executable homes to the launchd PATH without pinning a version or changing the registered command. `6bf4afb` changes only `plugin/.codex-plugin/plugin.json`'s version to `4.2.3-dev`. None creates a second transport or bypasses hook trust. |
