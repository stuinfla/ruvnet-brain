...
| 2026-09-17 | Re-read all 12 expanded governed paths, including experience scenarios/reporting, CI workflow lanes, QE scripts, and UX tests. The architecture remains the host/OS/install journey contract; source presence does not establish an exact-SHA hosted or public-artifact result. Source-bound review digest `21df81e00e0e`. | tests/experience/scenarios.json; tests/experience/report.mjs; .github/workflows/ci.yml; scripts/qe/ux-suite.mjs; tests/ux/render-probe.mjs |
...
| 2026-08-30 | The release candidate workflow now restores the source-bound coverage projection before archive sealing. | `.github/workflows/ci.yml` runs `scripts/release-projection.mjs` and rebuilds the archive with the projection, so release-QE validates the same coverage files that ship. |
