...
| 2026-08-23 | The Windows candidate lane serializes Vitest worker execution for its installer-import suites. | Hosted run `32654414505` isolated two concurrent installer-import suites with no failed assertions while five suites passed 33 tests; the platform boundary is narrowed without skipping assertions. |
