...
|---|---|---|
| 2026-08-23 | Re-read the release workflow after the exact seed assembled without the private fence required by projection. | Commit `655f9c2` copies the checked-in `kb/PRIVATE-STORES.json` into the extracted immutable seed before repair, bundle assembly, or projection; this restores the required security boundary without changing publisher authority or claiming publication. |
