...
|---|---|---|
| 2026-09-20 | Currency review, Dream Cycle cross-host-conformance rotation: decision unchanged; the payload-self-containment invariant this ADR gates is not weakened. `hook-registry.mjs`'s header comment cited `route-dispatch`/`learn-capture` as example ids `codex-hooks.json` currently dispatches; both stopped being live Codex examples at commit `00526b12` (2026-09-07, "retire automatic hooks" — wiped `codex-hooks.json` to `{"hooks": {}}`), not the 2026-09-09 restore (`56420430`) an earlier draft of this same fix mis-cited (caught by an independent critic agent's own git-history trace, not self-caught — see Reward-Hack/Security note in the committed report). Both ids remain valid `hook-shim.mjs` TABLE keys, just no longer Codex-dispatched. Corrected the example ids to the current live set (`session-start`, `decision-gate`, `grounding-stamp`, …) and the commit/date attribution, re-verified programmatically via `node plugin/scripts/hook-registry.mjs --machine=0`, `hook-shim.mjs`'s `TABLE`, and `git show <sha>:plugin/hooks/codex-hooks.json` at both commits. Comment-only — no payload membership, import, or shim-routing behavior changed; `tests/unit/payload-self-contained.test.mjs` and `tests/unit/hook-registry-lint.test.mjs` both re-run green, byte-identical pass/skip counts before and after. | Read `plugin/scripts/hook-registry.mjs`, `plugin/scripts/hook-shim.mjs`'s TABLE, `plugin/hooks/codex-hooks.json`'s live dispatch tokens and its git history at commits `00526b12`/`56420430` directly; ran both named test files before and after. |
| 2026-09-11 | Currency review at commit 7296c984: decision unchanged; the payload-self-containment invariant holds — `tests/unit/payload-self-contained.test.mjs` re-run green on this tree for this review. Only `plugin/scripts/unprompted-runtime.mjs` moved since 306432a4: `ba8cb8ba` and `1ddf90f0` (pre-session worktree work — a fitting rUv capability recommended on an ordinary request; a wedged producer cannot take the turn) and `1380c98b` (explicit RUVNET_ADVOCACY_ROUTE_BUDGET_MS passthrough, a no-op); none adds an import outside the payload. `plugin/scripts/anticipate.sh`, `capability-registry.mjs`, `hook-registry.mjs` did not move. | Reviewed `plugin/scripts/unprompted-runtime.mjs`, `tests/unit/payload-self-contained.test.mjs`. reviewed_digest 12cc75a4448f. |
| 2026-09-11 | Currency review at commit 2eef2024: decision unchanged; the payload-self-containment invariant this ADR gates is not weakened. `741e980c` adds a new sibling import to `anticipate.sh`, `plugin/scripts/capability-routing.mjs` — but it is wrapped in try/catch and never `quit()`s if absent ("older packed hosts predate the optional routing receipt"), unlike the mandatory `capability-registry.mjs`/`goal-match.mjs` imports that DO `quit()` on failure; it therefore cannot reproduce this ADR's original silent-inert failure mode. Confirmed both `capability-routing.mjs` and the also-new `nightly-controller.mjs` (imported by `6a6ba72f`'s `capability-registry.mjs` change) exist under `plugin/scripts/` and are named in `tests/unit/payload-self-contained.test.mjs`. The rest of `6a6ba72f`'s change disables the `memory-distillation` capability's `turnOn` (same underlying `distill-project --restore` hardening reviewed under ADR-049) and delegates nightly-job detection to `nightlyStatus()` — capability-content changes, not payload-boundary ones. | Read both diffs in full; verified the two new sibling files exist on disk and are referenced by the self-contained test (`grep -c` = 2). |
