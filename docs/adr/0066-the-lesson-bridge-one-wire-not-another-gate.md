...
|---|---|---|
| 2026-08-13 | **A tag is no longer treated as a human act of ratification. Bridged rows now derive status from rUv's typed provenance.** | The bridge minted every tagged row `STATUS.RATIFIED`, commented "the tag on the row IS the human act of ratification". But `ruflo memory store --tags` runs in every session, from hooks and from the model itself, and `nightly-wrapper.sh` runs `lesson-bridge --apply` unattended — so any process able to write a `lesson-*` row could inject standing policy into every later session, stamped ratified. I hand-wrote such a row myself on 2026-08-13, which would have self-ratified. ADR-323's `provenance_type` already carried the distinction and the bridge already read it eight lines earlier for `origin`; `status` ignored it. Both now read the same fact: `user_claim` -> RATIFIED, everything else -> CANDIDATE, which `lessonsFor()` will not deliver and `makeLesson()` will not let block. Live split: 23 of 43 stay in force, 20 drop, each NAMED on stdout because going quiet about 20 lessons no longer firing is the larger failure. **DISPUTED, unresolved:** an adversarial review argues this moved the boundary between two equally model-writable fields (the `--provenance` flag is on the same command as `--tags`) while silencing 20 working lessons that have NO in-product path back — `pending()` excludes IMPORTED_OWNER and `ratify()` refuses it — and that it cancels ADR-067's ship-trigger fix for the two ship lessons named there, both of which are `agent_output`. Going to a second independent model before any further change; the cancellation fires only on the next `--apply`. |
