...
|---|---|---|
| 2026-08-21 | **Coverage remains available while the Brain is OFF; this is diagnostic transparency, not retrieval.** | `plugin/commands/coverage.md` explicitly invokes `plugin/scripts/coverage.mjs`, which reads only the installed KB's `COVERAGE.json` and emits its source inventory/statuses. The third Console page reaches the same installed ledger through `scripts/onboarding-console.mjs::gatherSourceCoverage()`. Neither path performs semantic retrieval, learning, advocacy, maintenance, or mutation, so silencing them would hide what is installed precisely when the user is auditing an OFF Brain. Missing or malformed coverage fails as unavailable rather than becoming a false empty/success state. The command declaration remains host boot-frozen, while the ledger is read per invocation/request; a plugin restart may expose a new command, but a KB refresh does not change the OFF contract. |
