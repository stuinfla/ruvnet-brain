...
|---|---|---|
| 2026-09-11 | Currency review at commit 7296c984: decision unchanged and strengthened. `scripts/sync-census.mjs` `7296c984`: the census WRITER now measures the installed brain (`storeRoot()`) instead of `<repo>/kb`, so writer and grader share one number — this ADR's one-producer rule applied to the corpus count, which had two producers reading two directories. `scripts/sync-version.mjs` `7313db9e` exempts MCP test fixtures from the version-literal scan. Noted for the owner, not fixed here: `plugin/.claude-plugin/plugin.json` carries a hand-typed 'across 77 rUv repositories' that is not on the census writer's surface list — a restated fact this ADR forbids. | Reviewed `scripts/sync-census.mjs`, `scripts/sync-version.mjs`. reviewed_digest c36a9b076f17. |
| 2026-09-11 | Currency review at commit 2eef2024: decision unchanged, both drift commits strengthen it. `tests/unit/no-restated-truth.test.mjs` (`6a6ba72f`) replaced the `sync-version-ignore` line-matcher with a real `@babel/parser` AST comment scan, closing string/template-literal spoofing holes the naive regex allowed — the same "a gate may never contain a copy of the truth it checks" rule, enforced more exactly. `scripts/sync-census.mjs` (`b1172a75`, in a commit titled "connect maintenance entrypoints honestly") now reports chunk/store census as explicit UNKNOWN when no `kb/*.big.rvf.idmap.json` artifacts exist, instead of silently treating an absent ledger as agreeing — this is exactly this ADR's own "never invent a number" principle, applied to a case the prior code missed. `scripts/sync-version.mjs` was not touched in this range. | Read both diffs in full (the `sync-census.mjs` diff required extracting both git blobs directly — git's binary heuristic misclassifies the file, though it is plain JS text). |
