...
## Currency log
| 2026-08-30 | Re-read after Dream Cycle 2026-08-30's `codex-hook-adapter.mjs` change; no observable, mutant or budget changed. | The candidate (issue #196, PR #197) moves the adapter's `CONTEXT_EVENTS` constant into a new pure sibling, `plugin/scripts/codex-hook-events.mjs`, so a test can import it without triggering the adapter's side-effecting top level. The adapter's runtime behavior, output contract, and every value this ADR governs (PLATFORM_BUDGET, watch-plane definitions, grading thresholds) are unchanged — this is a data-location refactor with new test coverage, not a behavior change. |
...
|---|---|---|
| 2026-08-23 | Reconciled the release-bundle fence regression fix with the quality contract. | `scripts/build-bundle.mjs` now carries the exact private-store fence into the candidate output, and its regression test proves the policy survives assembly; protected CI remains the release authority. |
| 2026-08-23 | Re-read the CI workflow after adding the seed-coverage preflight. | Commit `efcecad` adds a fail-closed release-QE assertion only; the 95 contract and its governed gates remain unchanged. |
| 2026-08-22 | Bound the two independent review receipts to the exact retrieval plan before the sole publisher may mutate either channel. | Commit `f06624a` passes `release-evidence/retrieval-canary-plan.json` to `independent-review-receipt.mjs verify-pair`; `tests/unit/protected-release-workflow.test.mjs` proves this check precedes `release.mjs --publish`. The current verifier does not yet implement the S-10 row-relevance contract, so publication remains fail-closed and no D7/D8 or 95 score is claimed. |
| 2026-08-22 | Re-read the installer, dual-host hooks, release transaction, and public-verification changes at convergence tip `ddae606`; no critical invariant, mutant, exact-SHA boundary, or degraded-claim ban was removed. | `620d7bc` prevents tests from mutating the real nightly service, `adeba05` restores durable progression through both host manifests, and `3489b6e` + `4a901d6` + `ddae606` require independent signed reviews, public 3-by-3 host lanes, a signed aggregate, and a persisted install-verified receipt after channel convergence. Focused exact-tip gates pass, but real final-candidate reviews and public installed bytes have not run; this is not a 95 score and no public verification is claimed. |
...
| 2026-08-19 | **The `check` job's hang is identified and quarantined, so the job can pass again.** | `check`'s nine-day hang now has a NAME: `tests/unit/decision-outcomes.test.mjs`, identified by construction rather than guessed. `--no-file-parallelism` makes completion order equal execution order, and vitest sequences by file size DESCENDING — verified, not assumed: the 129 files that completed are EXACTLY the largest 129 by size, zero mismatches, so the file that started and never finished is the 130th. It converges with the independently-derived regression window (main last green 2026-08-10T11:06, first red 16:23), which contains the two ADR-067 decision-gate commits — and this is a decision-gate test. The file is QUARANTINED via `--exclude`, not fixed: it passes locally in 1.8s WITH coverage, so the cause is ubuntu-specific and unknown. #145 stays open; the exclusion comes out when the mechanism is understood, because a quarantine that quietly becomes permanent is deleted coverage. Two prior diagnostics failed and are recorded on #145: a global `child_process` patch would have silently no-opped (ESM binds a builtin's named exports at facade instantiation), and `--reporter=verbose` reveals nothing because vitest buffers a file's output and flushes it on COMPLETION. |
| 2026-08-19 | **The `darkStores()` alias-blindness recorded here as open is CLOSED, and so is its twin.** | `darkStores()` compared store names to card HEADINGS while the router resolves through `repositoryNames()` first, so a store reachable ONLY under an alias was reported dark — `metaharness` being the standing example, reached through the `agent-harness-generator` card. The false positive did real damage: acting on it, a DUPLICATE `## metaharness` card was added, it collided with `agent-harness-generator`, and alias resolution broke outright. The detector manufactured work that damaged the thing it detects. The same defect was found the same day in `scripts/brain-score.mjs`'s `routable` metric, which under-reported the brain's own reachability (30 stores called unroutable when 26 were). Both now resolve through kb/card-lane.mjs's `repositoryNames` — the router's OWN resolver, imported rather than reimplemented — and both carry a guard proven to go red on the old comparison and to REFUSE a fix that flatters (a store with no card and no alias must still read dark). Neither module is in the plugin payload (`marketplace.json` ships only `plugin/`), so this adds no reachable-module risk. Measured after: 29 dark stores, `metaharness` no longer among them. |
| 2026-08-19 | **The `check` job still cannot pass, and the reason is now bounded and being named.** | A temporary `--reporter=verbose` diagnostic was added to the `check` job's vitest step (#145). The step hangs DETERMINISTICALLY on ubuntu — two runs 38 minutes apart went silent after the same file, with the same six files before it and the same orphan signature at cleanup — and it survived every one of today's twelve unit-test fixes, so the failing tests were not the cause. The default reporter prints on file COMPLETION only, so a file that starts and never finishes is structurally invisible; verbose prints start events. Diagnostic only, to be removed once the file is named. The controlled comparison is already in the file: `windows-unit` runs the SAME directory WITHOUT `--coverage` and completes, `check` runs it WITH coverage and hangs; macOS with coverage completes locally (3334 passing), so coverage alone is not sufficient and the OS confound is unresolved. |
| 2026-08-19 | **The release chain could not seal, and CI could not fail — both fixed; the 5s PreToolUse contract on line 276 is honoured again.** | `release-qe` had refused every candidate since 2026-08-08 with `INVALID_LINEAGE` and named nothing (46 commits, nothing published, #141). Taught to name its cause, it answered instantly: `M data/org-repo-count.json` — `orgRepoCount()` persisted on every call and both callers are BUILD scripts, so the build dirtied the tree seconds before the seal demanded it clean. Persistence is now opt-in; the committed record still ships as the offline fallback. Separately, no job in ci.yml declared `timeout-minutes`, so a wedged vitest step ran toward GitHub's 6-hour default — EIGHT consecutive main runs sat in_progress for up to three hours, and because logs are unreadable while a run is in progress, the hang HID release-qe's and windows-unit's already-red results behind it. All four jobs now carry ceilings. Also: the two decision-gate PreToolUse entries returned from 10s to the 5s this document specifies (see ADR-067). |
| 2026-08-19 | **A shipped capability was unreachable on every install, and the 95 contract's own measurement helped hide it.** | `build-bundle.mjs` never copied `repo-aliases.json`, which `card-lane.mjs` reads to resolve a PRODUCT name to its installed store (agent-harness-generator -> metaharness, ruvnet-brain -> brain, ruvector -> rvf, agentic-qe -> qe). Measured against the live store root: file ABSENT, and `search_ruvnet("metaharness: what does Darwin mode actually mutate?")` answered "Searched 0 RuvNet repos". After placing it, the identical query returns cited ADR-070/081/105 with routing.accepted=true. `forge-update.mjs:505` already managed the file — the UPDATER knew and the BUILDER did not. THE MEASUREMENT PART, which belongs to this contract: `darkStores()` compares store names to card HEADINGS and is blind to aliases, so it reported metaharness as dark. I trusted that number and 'fixed' it by appending a duplicate `## metaharness` card, which then collided with the real card and broke alias resolution outright — a regression caught by windows-unit. A coverage metric that cannot see the alias layer produced a false gap, and acting on the false gap damaged the real thing. The duplicate is removed; darkStores() is still alias-blind and is recorded as open rather than quietly left. |
...
| 2026-08-06 | **The final release dispatch is delegated to the nightly; the release AUTHORITY is not.** New `scripts/release-convergence-watchdog.mjs`, invoked from `scripts/nightly-wrapper.sh`. | The published surfaces still name different generations (npm 4.0.12 vs GitHub v4.0.7) and the rail that converges them is now repaired, but GitHub Actions entered a MAJOR OUTAGE (`Actions: major_outage`, incident "workflow runs are still failing or delayed in starting", 0 running / 22 queued) while the maintainer left for a month. The publisher is an Actions workflow, so the last step could not be taken by hand. The watchdog DISPATCHES `protected-release.yml` and publishes nothing itself — `scripts/self-update.mjs:56` still refuses `--publish`, and every gate this ADR depends on (exact-SHA evidence, clean worktree, release-proof, host verification, post-publication seal) continues to run inside the workflow. It stands down with a stated reason on divergence-absent, stalled-Actions, open-PRs, dirty-tree, `-dev` version, missing `release()` commit, or absent green exact-SHA `ci`/`release-aggregate` — and exits 0 in every one of those cases, because it runs unattended for weeks and must never page anyone. Verified report-only the night it was added. Commit `6346721`. |
| 2026-08-06 | **Version bumps no longer count as governed-code drift** (`scripts/doc-currency.mjs`). | `sync-version.mjs` rewrites the plugin manifests, `package.json`, `kb/package.json` and `RVF-GENERATIONS.json` on every bump, and several ADRs legitimately govern those files — so every release bump marked them `presumed-stale`. Measured four times in one day (ADR-050/051/057/058), each resolving to "re-read, only a version string moved". A gate that fires on every bump teaches stamping without reading, which this repo has already done once across 61 ADRs, and it would have blocked the unattended release watchdog above, which cannot author a currency row. Drift now counts only commits whose diff inside the governed paths contains at least one non-version line; a commit touching one substantive line still counts in full. This narrows the trigger, never the verdict. |
| 2026-08-06 | **The sole-publisher contract this ADR relies on was NOT RUNNABLE, and is now repaired.** `protected-release.yml` gains its missing `EXPECTED_VERSION` definition, and `release-identity-invariants` gains a release-commit exemption. No claim in this ADR is weakened; one it silently depended on is restored. | This ADR's "protected sole publisher" language assumes the protected workflow can actually publish. Measured today it could not, twice over. (a) `da42a21` replaced `EXPECTED_VERSION: 4.0.8` with `REQUESTED_VERSION` in the env block but left all five uses on the old name (`protected-release.yml` lines 64, 158, 171, 174, 175); under `set -euo pipefail` the first check aborts on an unbound variable, so the only workflow permitted to sign and publish failed before comparing anything. (b) `protected-release.yml:170/183/186/187` require the receipt, manifest and tag to equal `package.json` at the candidate SHA — so a candidate must carry the CLEAN version — while `tests/unit/release-identity-invariants.test.mjs` asserted main is always `-dev`, making exact-SHA `ci` red on precisely those commits, which the same workflow then refuses. Every release candidate was red by construction. This is the mechanism behind #77: with the rail dead, releases were hand-made, and npm reached 4.0.12 while `releases/latest` stayed v4.0.7 (`scripts/published-surface-probe.mjs` D-version-coherence, FAIL). The exemption is derived from the commit subject, never a skip flag, and was verified across all four states — `-dev` passes, clean-on-ordinary-commit still fails, clean on `release(4.0.18)` passes, clean on `release(4.0.99)` fails. Commit `66e71ea`. |
...
Implementation note (2026-08-23): release projection and cross-host verification changes remain subject to the exact CI contract; production is not certified until all gates pass.
