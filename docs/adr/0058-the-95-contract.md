...
| 2026-09-17 | Re-read all 37 resolved governed entries against the current integration working tree, including the release vector, CI workflows, hook planes, installer, oracle and QE paths. The contract remains a proposed evidence contract; current source review does not award a 95 or establish public release qualification. Source-bound review digest `cd5d1cc0cf29`. | scripts/release-vector.mjs; scripts/release-proof.mjs; scripts/claims-verify.mjs; scripts/no-silent-substitution.mjs; .github/workflows/ci.yml; .github/workflows/protected-release.yml; plugin/hooks/hooks.json; plugin/hooks/codex-hooks.json |

| 2026-09-16 | Corrected the document update date after the QA stamp check found it lagged the recorded September edits. This is date metadata repair, not a new implementation review or conformance claim. | `scripts/doc-currency.mjs`; ADR-087 tracks the active source review and validation. |
...
| D7 −30 "regex parses shell semantics" | **Closed at the authorization boundary.** Managed CLI help/run uses structured schemas and literal argv; the former raw-shell hook is retired with a silent compatibility ID. The historical shell corpus remains a no-dispatch regression, and canonical MCP tests own the live boundary. | Packed/published-host proof remains separate under D8. |
