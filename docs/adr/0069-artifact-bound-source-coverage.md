...
|---|---|---|
| 2026-08-21 | Recorded the implemented bundle/install projections, terminal command, and third Console page while keeping the full decision Proposed. | `scripts/build-bundle.mjs`, `plugin/commands/coverage.md`, `plugin/scripts/coverage.mjs`, `console/coverage.html`, `scripts/onboarding-console.mjs`, and focused coverage tests now expose installed `COVERAGE.json`; signed enumeration, closed-snapshot proof, signed exemptions, and public/install coherence remain absent. |
