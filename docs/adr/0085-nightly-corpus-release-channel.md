...
status: Proposed  # 2026-09-19: scheduled dispatcher exists, but the only green scheduled run was disarmed and the latest real corpus candidate failed on a gist-detail HTTP 403. No successful end-to-end nightly refresh is proven. This ADR's 2026-09-13 history is retained as dated history; current state is recorded below.
date: 2026-09-12
updated: 2026-09-19
...
| Read-only preparation workflow | Built and callable manually or by the protected workflow. It has no cron itself. | `.github/workflows/corpus-seed.yml` — `workflow_dispatch`/`workflow_call` |
| Scheduled dispatcher | A 07:17 UTC daily cron exists and checks that the approved runtime is present before dispatch. Run 35429366082 completed green while explicitly DISARMED and dispatched nothing. | `.github/workflows/corpus-nightly-dispatch.yml`; run 35429366082 |
| End-to-end corpus outcome | Manual corpus run 35463759285 failed while fetching gist `1afab76a2b67161b7bd1fbdd8930b408`: public detail returned HTTP 403 after roughly 70 minutes of build work. A dispatcher success currently proves dispatch creation only; the workflow is being changed to await and report the protected child conclusion. | `corpus-failed.log`; run 35463759285; this ADR's 2026-09-19 currency entry |
...
Historical decision: do not build a second publisher; use the canonical protected-release path. As of 2026-09-19, `.github/workflows/corpus-nightly-dispatch.yml` already has the daily dispatcher and `protected-release.yml` remains the child path. The remaining production gap is successful end-to-end acquisition and qualification, not the existence of a schedule. The old 2026-09-13 decision and currency rows below describe the state on that date.
