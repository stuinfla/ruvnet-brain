...
|---|---|---|
| 2026-09-12 | **Two sessions fired on the same SLOT=2/DEEP (`enforcement-integrity`) night and independently wrote byte-identical production fixes for the same pre-existing defect (#264).** The second session to attempt a push discovered the collision only when `git push` was rejected (the branch name it had independently chosen already existed on `origin` with different history) — not predicted in advance. `dream.config.json`'s own `findingPolicy.skipIf: ["existing-fix-pr"]` is the discipline this ADR relies on to prevent exactly this: a duplicate candidate PR. The second session verified the duplication by diff (not assumption), stood down from opening a second PR, and instead reconciled 6 other accumulated open issues on the same surface (#156/#158/#181/#183/#228/#230) that the first session's PR had not checked, closing each with source-citing evidence. | PR #281 (first) and this ADR's `LEDGER.md` row for 2026-09-12 (second, reconciliation-only). `dream.config.json`'s `findingPolicy` object is not enforced by the `dream-machine` compiler (a pre-existing, documented gap — see LEDGER.md's own Operating policy note) but was followed here as a repo-level convention regardless, exactly as this ADR's `extraDisciplines` override already relies on convention over compiler enforcement. |
