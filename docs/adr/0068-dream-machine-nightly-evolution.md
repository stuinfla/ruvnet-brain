...
|---|---|---|
| 2026-08-22 | **MetaHarness preflight is now an explicit, read-only gate for evolution evidence.** | `scripts/metaharness-gate.mjs` composes measured score, OIA severity, drift, and routing receipts without subprocesses or metered calls. `scripts/routing-flywheel.mjs` refuses the live proposer unless a receipt explicitly authorizes metered spend; synthetic replay remains $0. Promotion requires a clean measured preflight and a verified synthetic replay candidate. Missing/degraded evidence blocks rather than becoming a fabricated score. |
| 2026-08-22 | **Separated Dream evaluation from the retired local source-writing nightly.** | `com.ruvnet.brain-nightly` was a different mechanism from this ADR's cloud routine: it ran `scripts/nightly-wrapper.sh` in the primary developer checkout and accumulated generated changes beside active work. It is now unloaded, archived, removed from `config/scheduled-jobs.json`, and absent from `deploy/`. This ADR's routine remains the evaluation path and keeps `autoMerge:false`; it did not acquire source mutation or publication authority. The supported `com.ruvnet.brain-update` job updates installed bytes only. |
