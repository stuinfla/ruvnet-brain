...
| 2026-09-07 | Night 2026-09-07 (SLOT 2, deep `enforcement-integrity`) ran and appended `docs/dream-cycle/LEDGER.md`'s next row (#262/#263, ACCEPT: `lesson-promote.mjs`'s theme-demotion control was unreachable dead code, fixed by following `lesson-gate.mjs`'s `OPTIN_PATH` precedent), then amended that same row twice more as real CI on the PR surfaced things the local evaluation missed — an independent critic's verdict, and a `windows-unit` cross-platform test bug (`os.homedir()` reads `USERPROFILE` on win32, not `HOME`). Each `LEDGER.md` edit is itself a write to this ADR's `governs:`-listed path, so this row is touched in the same commit each time rather than left to `presumed-stale` again. Also the direct cause this row first existed: `doc-currency.mjs`'s own `canonical-qa` CI check flagged the PR's first `LEDGER.md` edit as `presumed-stale` against this ADR's own then-stale `updated:` stamp — the same drift-detection class this ADR's other 54 sibling violations already represent repo-wide, caught fresh on this session's own change rather than left to accumulate as a 55th. `autoMerge: false` held; draft PR, human review required. | b3e975f, 063e290, dce07d8 |
