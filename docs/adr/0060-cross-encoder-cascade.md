...
|---|---|---|
| 2026-09-19 | Currency review for ADR-090 at source digest `dc868233b250`: `kb/forge-ask-all.mjs` and `kb/forge-mcp-all.mjs` add a finite query-template lane backed by exact reviewed source-passage SHA-256 values, with null CE scores and explicit runtime-unverified caveats; MCP bypasses curated answer cards for those queries. Changed or missing source passages fall through. Cascade stages, pool size, timeout, CE threshold, default `k`, and worker protocol are unchanged. `kb/forge-rerank.mjs`, `scripts/rerank-cap-warm-ab.mjs`, and `scripts/rerank-cap-eval.mjs` did not change. See ADR-090 for the source-catalog contract. | Reviewed the retrieval and MCP code changes; confirmed no cascade settings or worker protocol changed. |
| 2026-09-12 | `kb/forge-ask-all.mjs`'s post-rerank `selectResults()` moved: the negative-cross-encoder-score prune (`kept = results.filter((r,i) => i===0 \|\| ceScore>=0)`) now also exempts a candidate the repo-name-affinity boost already matched (`r.nameBoosted`). Found live: a query naming its repo explicitly (every "In the &lt;repo&gt; repository, ..." retrieval-canary case) still had that repo's own verified-correct passage discarded when it scored negative on the cross-encoder's absolute, post-boost scale. Does not touch `CE_CASCADE_K_DEFAULT`, the cascade stages, or the worker protocol — the cascade still produces the same scored pool; this changes only which already-scored candidates survive selection. `kb/forge-rerank.mjs`, `scripts/rerank-cap-warm-ab.mjs` did not move. | An earlier version of this exemption gated on the search LANE (`fullCorpusLane`) instead of the candidate; Dual review (Fable 5.1 + Astra GPT-6) found the live `search_ruvnet` MCP path always calls `searchAll` with `allowFullCorpus:false`, where that lane is unreachable, so the lane-based version would have silently disabled this filter on every production query. Gating on `r.nameBoosted` instead applies in every lane, to only the specific candidates whose own repo the query names. reviewed_digest f00b91f1cf09. |
