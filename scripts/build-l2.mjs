...
for (const t of TOPICS) {
  ...
  let realCited = countRefs(article, paths);
  if (realCited.length < 2) { article = await gen(' YOUR LAST ATTEMPT CITED TOO FEW REAL FILES — REJECTED. Cite at least 3 of the exact SOURCE PATHS listed above, verbatim, in backticks.'); realCited = countRefs(article, paths); }
  const accepted = isGrounded(realCited, 2);                 // ADR-0002 R1: enforce grounding or REJECT
  writeGated({
    liveDir: path.join(ROOT, 'kb', 'l2'),
    rejectedDir: path.join(ROOT, 'kb', 'l2', 'rejected'),
    filename: `${t.slug}.md`,
