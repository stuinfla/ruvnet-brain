...
export function main(argv=process.argv.slice(2), env=process.env) {
  try {
    const sha=arg(argv,'--candidate-sha'), fable=arg(argv,'--fable'), astra=arg(argv,'--astra'), identity=arg(argv,'--identity');
    if (!/^[a-f0-9]{40}$/.test(sha||'') || !fable || !astra || !identity) throw new Error('usage: --candidate-sha <sha> --fable <receipt> --astra <receipt> --identity <json>');
