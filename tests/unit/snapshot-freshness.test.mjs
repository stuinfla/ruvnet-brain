...
describe('newestSnapshot()', () => {
  ...
  it('rejects empty files, directories and symbolic links masquerading as snapshots', ({ skip }) => {
    fs.writeFileSync(path.join(dir, 'empty.db'), '');
    fs.mkdirSync(path.join(dir, 'folder.db'));
    expect(newestSnapshot(dir, Date.now())).toBeNull();
    const target = touch('outside.txt', Date.now());
    try { fs.symlinkSync(target, path.join(dir, 'linked.db')); }
    catch (error) { if (error.code === 'EPERM' && process.platform === 'win32') return skip('Host cannot create symlinks'); throw error; }
    expect(newestSnapshot(dir, Date.now())).toBeNull();
  });
});
