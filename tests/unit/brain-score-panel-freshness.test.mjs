 ...
 *
 * Measured 2026-09-11, this container: `data/grade-*.json` mtimes are all today's checkout
 * timestamp (`git log -1` on each shows a real last-touched date weeks earlier), and
 * `node scripts/brain-score.mjs` reported `panelStrict ... 0d old` — a fabricated freshness
 * reading. `git clone`/`checkout` resets every file's mtime to the checkout instant; a panel
 * graded weeks or months ago reads as brand new on any fresh checkout, CI runner, or this very
 * nightly agent's own ephemeral container. That is exactly the "quoted a 34-day-old 100/100 as
 * current" defect this file's own header names as the reason `maxAgeDays`/`staleness()` exist —
 * alive in the one dimension that never consulted them.
 */
const tmpDirs = [];
