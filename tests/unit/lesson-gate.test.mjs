...
describe('BUDGET: an opted-in BLOCK survives cross-trigger truncation', () => {
  ...
  test('the block is present even when it is the lowest-repeatCount candidate in the whole merge', () => {
    writeStore([
      blockLesson({ id: 'T08-loud', trigger: 'claim-done', enforcement: 'checklist', check: null, repeatCount: 50 }),
      blockLesson({ id: 'T09-loud', trigger: 'report-status', enforcement: 'checklist', check: null, repeatCount: 40 }),
      blockLesson({ id: 'T01-verify-with-a-capable-channel', trigger: 'write-code', repeatCount: 1 }),
    ]);
    writeOptIn(['T01-verify-with-a-capable-channel']);
    const { code } = runGate([
