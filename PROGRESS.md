...
- **Remaining before push:** currency reviews on the ~16 in-force ADRs whose governed code moved (in flight, judgment rows with commit SHAs — not `--fix`); `npm run convergence:write`; full `vitest run tests/unit` green; then push (marketplace customers pull `main` and currently hold the e1a3c15 hooks.json).
